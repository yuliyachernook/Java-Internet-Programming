...
public class AddLink extends HttpServlet implements Servlet {
    ...
    protected void doGet(HttpServletRequest rq, HttpServletResponse rs)
        ...
        rs.sendRedirect("http://localhost:8081/AS_3022CYS25/index.jsp");
    }
    ...
    protected void doPost(HttpServletRequest rq, HttpServletResponse rs)
        ...
        if (Ref.length()>1 && Description.length()>1) {
            try {
                Connection connection = JdbcConnection.getConnection();
                Statement statement = connection.createStatement();
                statement.executeUpdate(String.format("INSERT INTO Links (url, description, minus_counter, plus_counter ) Values (\'%s\',\'%s\',\'%d\',\'%d\')", Ref, Description, 0,0));
                connection.close();
            } catch (SQLException var12) {
                System.out.println(var12);
            }
