...
public class Link {
    ...
    public Link(int id, String ref, String descriptionLink, int isuseful, int isuseless) {
        this._id = id;
    ...
    public Link(int id, String ref, String descriptionLink) {
        this._id = id;
        this._DescriptionLink = descriptionLink;
        this._IsUseful = 0;
        this._IsUseless = 0;
    ...
    public static Link getLinkFromDb(int Id) throws SQLException, ClassNotFoundException {
        Connection connection = JdbcConnection.getConnection();
        Statement statement = connection.createStatement();
        ResultSet result = statement.executeQuery("SELECT * FROM Links WHERE link_id = " + Id);

        Link link;
        for(link = null; result.next(); link = new Link(result.getInt(1), result.getString(2), result.getString(3), result.getInt(4), result.getInt(5))) {
        }
